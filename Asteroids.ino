...
public:
  ...
  void select_font(const unsigned char [])       { /* TODO */ }
  void set_vbi_hook(void (*dummy)())             {}
  void set_pixel(uint8_t x, uint8_t y, char c)   { arduboy.drawPixel(x, y, c); }
  void delay(unsigned int x)                     { delay(x); }
  void print(uint8_t x, uint8_t y, char* ss)     { arduboy.setCursor(x, y); arduboy.print(ss); }
  void fill(uint8_t color)                       { arduboy.fillScreen(color); }
  void print_char(uint8_t x, uint8_t y, char c)  { arduboy.drawChar(x, y, c, WHITE, BLACK, 1); }
  void draw_line(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, char c) { arduboy.drawLine(x0, y0, x1, y1, c); }
  void tone(unsigned int frequency, unsigned long duration_ms) { arduboy.tunes.tone(frequency, duration_ms); }
};
TVout tv;

class TVController {
public:
  uint8_t leftPressed()  { arduboy.pressed(LEFT_BUTTON); }
  uint8_t upPressed()    { arduboy.pressed(UP_BUTTON); }
  uint8_t rightPressed() { arduboy.pressed(RIGHT_BUTTON); }
  uint8_t downPressed()  { arduboy.pressed(DOWN_BUTTON); }
  uint8_t firePressed()  { arduboy.pressed(A_BUTTON); }
};
TVController Controller;

// END OF ARDUBOY COMPATIBILITY LAYER
// -------------------------------------------------------------
...
//prog_char *strings[] PROGMEM = {s0, s1, s2, s3};
...
void gameOver() {
  ...
  strcpy_P(s, (char *)pgm_read_word(&(s2)));
  tv.print(40, 40, s);
  strcpy_P(s, (char *)pgm_read_word(&(s3)));
  tv.print(72, 40, s);
  tv.delay(2000);
}
...
void enterInitials() {
  ...
  tv.fill(0);
  strcpy_P(s, (char *)pgm_read_word(&(s1)));
